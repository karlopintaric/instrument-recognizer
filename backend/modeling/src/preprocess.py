...
class IRMASPreprocessor:
    def __init__(self, base_dir, new_dir, sr,
        ...
        if metadata is None:
            try:
                ...
                warnings.warn(
                    "No metadata csv found. New metadata will be generated when you first run the preprocessor.")
                self.metadata = None
        else:
        ...
        self.instruments = [
            inst.stem for inst in self.base_dir.iterdir() if inst.is_dir()]
    ...
    def preprocess_and_combine_all(self, n_jobs: int = -1):
        ...
        Parallel(n_jobs=n_jobs)(delayed(self._combine)
    ...
    def _get_filepaths(self, instr):
        ...
        files = metadata.apply(
            lambda x: f'{self.base_dir}/{x.instr}/{x.fname}.wav', axis=1).to_numpy()
        return files
    ...
    def _combine_files_and_save(self, instr1_files, instr2_files, save_dir):
        ...
        for (file1, file2) in zip(instr1_files, instr2_files):
            ...
            sf.write(os.path.join(save_dir, new_name),
                     synced_file, samplerate=self.sr)
    ...
    def _sync_and_combine(self, base_file_path, file_to_sync_path):
        ...
        file_to_sync, sr_sync = librosa.load(
            file_to_sync_path, sr=None, mono=True)

        if sr != sr_sync:
            file_to_sync = librosa.resample(
                y=file_to_sync, orig_sr=sr_sync, target_sr=sr)
        ...
        base_metadata = self.metadata.loc[self.metadata.fname ==
                                          base_file_name, cols]
        if not base_metadata.empty:
            pitch_base, bpm_base, onset_base = extract_from_df(
                base_metadata, cols)
        else:
            raise KeyError("File not found in metadata. Please regenerate")

        file_to_sync_metadata = self.metadata.loc[self.metadata.fname ==
                                                  file_to_sync_name, cols]
        ...
        if self.sync_pitch:
            file_to_sync = self._sync_pitch(
                file_to_sync, sr, pitch_base, pitch=pitch)

        if self.sync_onset:
            file_to_sync = self._sync_onset(
                file_to_sync, sr, onset_base, onset=onset)
    ...
    def generate_metadata(self, save_path: str = ".", n_jobs: int = -1):
        ...
        output = Parallel(n_jobs=n_jobs)(delayed(self._get_file_info)
