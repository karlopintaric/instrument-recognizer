...
def load_model():
    ...
    model.load_state_dict(torch.load("./models/acc_model_ast.pth", map_location=torch.device('cpu')))
    model.eval()
    return model
...
def load_thresholds():
    thresholds = np.load("./models/acc_model_thresh.npy", allow_pickle=True)
    return thresholds
